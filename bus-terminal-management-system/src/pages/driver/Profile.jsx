...
import { FiSave, FiLock, FiMail, FiPhone, FiTruck, FiCalendar, FiStar, FiNavigation } from 'react-icons/fi';
...
import { updateMyDriverProfile } from '../../services/driverService';
...
const ProfileForm = ({ user, onSaved }) => {
  ...
  const onSubmit = async (values) => {
    try {
      const { licenseNumber, licenseExpiry, ...userFields } = values;
      const [updated] = await Promise.all([
        updateProfile(userFields),
        updateMyDriverProfile({ licenseNumber, licenseExpiry }),
      ]);
      onSaved({ ...updated, licenseNumber, licenseExpiry });
    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-1">
          <div className="flex flex-col items-center text-center">
            ...
            <div className="mt-6 w-full space-y-3 border-t border-slate-100 pt-4 text-left text-sm">
              ...
              <div className="flex items-center gap-2 text-ink-variant">
                <FiTruck className="h-4 w-4 text-ink-muted" />
                <span>Bus {user.assignedBusId ?? 'Unassigned'}</span>
              </div>
              <div className="flex items-center gap-2 text-ink-variant">
                <FiStar className="h-4 w-4 text-ink-muted" />
                <span>{user.rating ? `${user.rating.toFixed(1)} / 5 rating` : 'No rating yet'}</span>
              </div>
              <div className="flex items-center gap-2 text-ink-variant">
                <FiNavigation className="h-4 w-4 text-ink-muted" />
                <span>{user.totalTrips ?? 0} trips completed</span>
              </div>
              <div className="flex items-center gap-2 text-ink-variant">
        ...
        <div className="space-y-6 lg:col-span-2">
          <Card title="Personal Information" subtitle="Update your contact details and license information.">
